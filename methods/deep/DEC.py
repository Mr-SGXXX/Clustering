...
class DEC(DeepMethod):
    def __init__(self, dataset, logger: Logger, cfg: config):
        ...
        self.weight_dir = cfg.get("DEC", "weight_dir")
    ...
    def train_model(self):
        ...
        with tqdm(range(self.train_max_epoch), desc="Clustering Training", dynamic_ncols=True, leave=False) as epoch_loader:
            for epoch in epoch_loader:
                ...
                y_pred = torch.argmax(q, dim=1).cpu().detach().numpy()
                delta_label = torch.sum(y_pred != y_pred_last).item() / y_pred.shape[0]
                y_pred_last = y_pred
                ...
                for data, _, idx in train_loader:
                    data = data.to(self.device)
                    x_bar, q, z = self(data)
                    loss = nn.KLDivLoss()(q.log(), p[])
                    total_loss += loss.item()
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                total_loss /= len(train_loader)
                metrics.update_loss(total_loss)
                if epoch % 10 == 0:
